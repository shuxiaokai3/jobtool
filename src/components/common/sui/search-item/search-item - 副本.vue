...
<template>
    ...
    <el-col v-if="type !== 'custom'" :xs="24" :sm="24" :md="12" :lg="6" :xl="6">
        ...
        <el-form-item v-else-if="type === 'select' && !multi" :label="realLabel">
            <el-select v-model="formInfo[vModel]" :placeholder="placeholder" filterable :size="$root.VUE_BASE_CONFIG.styleConfig.size" class="w-100" clearable @change="handleChange">
                <el-option v-for="(item, index) in realSelectEnum" :key="index" :label="item[selectProps.name]" :value="item[selectProps.id]"></el-option>
            </el-select>
        </el-form-item>
        <!-- 多选框 -->
        <el-form-item v-else-if="type === 'select' && multi" :label="realLabel">
            <el-select v-model="multiData" :placeholder="placeholder" filterable :size="$root.VUE_BASE_CONFIG.styleConfig.size" class="w-100" clearable multiple @change="handleChange">
                <el-option v-for="(item, index) in realSelectEnum" :key="index" :label="item[selectProps.name]" :value="item[selectProps.id]"></el-option>
            </el-select>
        </el-form-item>
        <!-- 单日期 -->
        <el-form-item v-else-if="type === 'date'" :label="realLabel">
            <el-date-picker 
                    ...
                    type="date" 
                    :placeholder="placeholder" 
                    clearable :size="$root.VUE_BASE_CONFIG.styleConfig.size" 
                    class="w-100" 
                    @change="handleChange"
            >
            </el-date-picker>
