...
<template>
    <div v-loading="loading" :element-loading-text="randomTip()" element-loading-background="rgba(255, 255, 255, 0.9)">
        <el-table ref="s-table" v-bind="$attrs" :data="tableInfo" stripe border :size="$root.VUE_BASE_CONFIG.styleConfig.size" :height="tableHeight" v-on="$listeners" @selection-change="handleSelectionChange">
        ...
        <div v-if="plain !== true" class="d-flex j-end mt-1">
            <slot name="operation" />
            <el-button :loading="loading" type="primary" icon="el-icon-refresh" :size="$root.VUE_BASE_CONFIG.styleConfig.size" @click="getData">刷新</el-button>
            <el-button v-if="deleteMany" :loading="loading2" :disabled="selectData.length === 0" :title="disableTip" type="danger" icon="el-icon-delete" :size="$root.VUE_BASE_CONFIG.styleConfig.size" @click="deleteData">批量删除</el-button>
            <el-pagination 
                    class="ml-4" 
