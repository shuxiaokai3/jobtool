...
export default {
    ...
    data() {
        return {
            uploadPicUrl: process.env.NODE_ENV === "development" ? BaseConfig.httpConfig.devImgUrl : BaseConfig.httpConfig.proImgUrl, //文件上传url, //文件上传url
            activeFileUrl: "", //文件url
            activeFileType: "", //文件类型
    ...
    watch: {
        value: {
            handler(val) {
                ...
                //     this.fileList = [];
                //     this.imgResult = [];
                //     this.enableExistFile = true;
                //     return [];
                // } else {
                //     // const imgs = val.split(",").map(val => ({ url: val }));

                // }

                // console.log(this.existFiles, this.fileList, this.imgResult) console.log(this.existFiles, this.fileList, this.imgResult)
            },
            deep: true,
    ...
    methods: {
        ...
        async httpRequest(fileObj) {
            ...
            const fileUrl = this.aliOssConfig.dir + Date.now() + "_" + fileObj.file.name;
            formData.append("name", fileObj.file.name);
            formData.append("key", fileUrl);
            formData.append("policy", this.aliOssConfig.policy);
            formData.append("OSSAccessKeyId", this.aliOssConfig.accessid);
            formData.append("success_action_status", 200);
            formData.append("signature", this.aliOssConfig.signature);
            formData.append("file", fileObj.file);
            return new Promise((resolve) => {
                axios.post(fileObj.action, formData).then(() => {
                    resolve({ data: fileObj.action + "/" + fileUrl });
                }).catch(err => {
        ...
        getFileTicket() {
            return new Promise((resolve, reject) => {
                this.axios.get("/auth/aliyun/oss/getKey").then(res => {
                    this.aliOssConfig = res.data;
                    this.expireTime = parseInt(res.data.expire);
                    resolve();
                }).catch(err => {
                    this.$errorThrow(err, this);
                    reject(err);
