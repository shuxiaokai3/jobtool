...
<template>
    <div class="s-viewer">
        <div ref="wrap" v-loading="loading" :element-loading-text="randomTip()" element-loading-background="rgba(255, 255, 255, 0.9)" class="img-wrap" @mouseover.stop="showBtn = true" @mouseleave="showBtn = false">
            <img ref="img" v-drag :src="imgUrl" @load="handleImgLoad" @mousewheel="handleMouseWheel">
...
export default {
    ...
    computed: {
        imgCount() {
            const len = this.imgList.length === 0 ? 1 : this.imgList.length
            return len
        }
    },
    watch: {
