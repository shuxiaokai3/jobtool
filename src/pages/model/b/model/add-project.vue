...
<template>
    <el-dialog title="新增项目" :visible.sync="isShow" :before-close="closeModel" :close-on-press-escape="false" :close-on-click-modal="false" width="40%">
        <el-form ref="form" :model="formInfo" :rules="rules" label-width="150px">
            ...
            <el-form-item label="简介描述：" prop="remark">
                <el-input v-model="formInfo.remark" name="name" size="mini" placeholder="填写一些介绍" class="w-100" maxlength="100" clearable></el-input>
            </el-form-item>
            <el-form-item label="是否收藏：" prop="favorite">
                <el-radio-group v-model="formInfo.favorite">
    ...
    data() {
        return {
            ...
            rules: {
                ...
                remark: [{ required: true, message: "请填写项目介绍", trigger: "blur" }],
                favorite: [{ required: true, message: "请选择是否收藏项目", trigger: "change" }],
            },
            //=====================================其他参数====================================//
            loading: false
        };
        ...
        */
        handleAddProject() {
            this.$refs["form"].validate(valid => {
                if (valid) {
                    // this.loading = true;
                    // this.axios.post("/api/project/add_project", this.formInfo).then(res => {
